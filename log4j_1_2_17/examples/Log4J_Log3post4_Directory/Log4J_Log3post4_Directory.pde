/**
 * Log4J_Log2_Directory
 * implementation of log4j for processing 
 * This file is modest jog4j code 
 * laminated onto    examples/data/CharacterStrings
 *
 * It creates a    $SketchDirectory.log    file in c:\logs       on windows machines     (name never changes run after run)
 * It creates a    $SketchDirectory.log    file in /home/pi/logs on non-windows machines (name never changes run after run) 
 *
 * if the target directory does not exist, it will be created.
 *
 * it also creates a $SketchDirectory.post2.log file (name never changes run after run) in that same target directory after 4 characters have been typed. 
 *
 *  Thank you Jake Seigel for having been the pathfinder:   https://jestermax.wordpress.com/2014/06/09/log4j-4-you/
 */
import org.apache.log4j.*;
Logger logM  = Logger.getLogger("Master");
Logger log  = Logger.getLogger("Ancillary0");
...
char letter;
String words = "TypeMoreThan4Chars...";
String priorWords="";
int count=0;
...
void draw() {
  ...
  textSize(36);
  text(words, 50, 120, 1040, 300);
  
  /* need some criterion to prevent logging durring every execution of draw() */ 
  if(!words.equals(priorWords)){
    String msg=String.format("frameCount=%6d String is now %3d char long. String=%s",frameCount,words.length(),words);
    log.info(msg);
    priorWords=words;
    count+=1;
    if(5==count){
...
void initLog4j_1(){
  String masterPattern = "%-5p %8r %M - %m%n"; /* debug_level miliseconds Originating_Method  - message    */
  String[] commandWords=System.getProperty("sun.java.command").split(" ");
  String logFileNameEndPart=commandWords[commandWords.length-1]+".post4.log";
 
  FileAppender fa0 = new FileAppender(); /* fa0 is local to this method, so can use same name name as previously used in initLog4j() */ 
  fa0.setFile((System.getProperty("os.name").toLowerCase().startsWith("win")?"c:\\logs\\":sketchPath("/home/pi/logs/"))+logFileNameEndPart);  
  fa0.setLayout(new PatternLayout(masterPattern));
  fa0.setThreshold(Level.DEBUG);
  fa0.setAppend(false);
  fa0.activateOptions();  
  log1.addAppender(fa0); /* this causes the fa0 file to get only the content sent to log */
  
  log1.trace("log checking ability to write at trace level"); /* not believed to be part of log4j, included here for future compatiblity, as is IS part of log4j2  */
  log1.debug("log checking ability to write at debug level");
  log1.info ("log checking ability to write at info  level");
  log1.warn ("log checking ability to write at warn  level");
  log1.error("log checking ability to write at error level");
  log1.fatal("log checking ability to write at fatal level\n");
  
  try{    
    String pathOnward=System.getProperty("sun.java.command").substring(System.getProperty("sun.java.command").indexOf("path=")+5);
    String[] words=pathOnward.split(" ");
    /* trying to leave a trail of breadcrumbs as to the source of the log file */
    log1.info("this log file was created by "+words[0]+File.separator+words[words.length-1]+".pde\n");  
  } catch (Exception e) {     
    e.printStackTrace();
  }
}
